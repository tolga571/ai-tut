...
// For Paddle webhook signature validation
// In production, we'd use process.env.PADDLE_WEBHOOK_SECRET
// Webhook docs: https://developer.paddle.com/webhooks/overview
...
export async function POST(req: Request) {
  try {
    ...
    // Parse the payload (assuming validation passed for MVP)
    const payload = JSON.parse(body);

    const eventType = payload.event_type;
    const { id: paddleSubscriptionId, status, custom_data, customer_id } = payload.data;
