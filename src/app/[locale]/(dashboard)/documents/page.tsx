          ...
          <ul className="space-y-4">
            {posts.map((post) => {
              ...
              return (
                <li
                  key={post.id}
                  className={`bg-gray-50 dark:bg-gray-900 border rounded-2xl p-5 transition-all ${
                    locked
                ...
                >
                  {locked ? (
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-gray-900 dark:text-white font-semibold text-base">{post.title}</span>
                    ...
                    <Link href={`/documents/${post.slug}`} className="block group">
                      <div className="flex items-center gap-2 mb-1">
                        <h2 className="text-gray-900 dark:text-white font-semibold text-base group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors">
                          {post.title}
                        </h2>
                        {post.isPremium && (
                          <span className="px-2 py-0.5 bg-yellow-500/10 border border-yellow-500/30 rounded-lg text-xs text-yellow-400">
